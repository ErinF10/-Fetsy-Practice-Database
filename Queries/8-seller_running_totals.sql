...
DROP PROCEDURE seller_running_totals;
DELIMITER :
CREATE PROCEDURE seller_running_totals(IN sellerName VARCHAR(255))
BEGIN
	SELECT seller_id, order_id, order_date, 
		CONCAT('$', FORMAT(order_total, 2, 'en_US')) AS order_total,
        CONCAT('$', FORMAT(sales_total, 2, 'en_US')) AS sales_total
	FROM (
		SELECT seller_id, order_id, order_date, order_total, SUM(order_total) OVER
			(ORDER BY order_date ROWS UNBOUNDED PRECEDING) AS sales_total
		FROM (
			SELECT (orders.order_quantity * products.product_price)/100
				AS order_total, sellers.seller_id AS seller_id, seller_name,
                orders.order_id AS order_id, order_date
			FROM sellers
